...
function pages($num, $curr_page, $perpage = 20, $urlrule = '', $array = array(),$setpages = 10) {
	...
	if($num > $perpage) {
		...
		$multipage .= '<div><a class="a1 totalnum">'.$num.'</a><a class="oneshow">'.$perpage.'</a><a class="nowshow">'.$curr_page.'</a> ';
		if($curr_page>0) {
		...
		if($curr_page<$pages) {
			...
			$multipage .= ' <a href="'.pageurl($urlrule, $pages, $array).'">'.$pages.'</a> <a href="'.pageurl($urlrule, $curr_page+1, $array).'" class="a1">'.L('next').'</a></div>';
		}
	}
	return $multipage;
}
