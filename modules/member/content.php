		...
		if(isset($_POST['dosubmit'])) {
            ...
            $totalmoney=0;
             foreach ($arr as &$value) {
                 ...
                 $content = $content_db->get_one(array('id'=>$pid));
                 $totalmoney+=$content['shop_prize']*$value->buy;
             }

			if($totalmoney!=$_POST['info']['shop_total']){
                alert::message(-1,'請勿修改商品資訊');
			}
			...
			foreach ($arr as &$value) {
                ...
                foreach ($list as &$value2) {
                    if ($value2->id==$sid){
						...
						if($value2->number<$value->buy){

							alert::message(-1,'數量已經超過囉');
						}
                    }
                }
            }
			...
			if($info['status']==99) {
                ...
                if($type){
                    alert::message(1,'送出成功,請再6個小時內轉帳',$id);
                }else{
	...
	public function delete(){
		...
		if(!$array){
			...
			$table_name = $content_db->table_name;
 			$content_db->delete_content_orderlist($id); //删除文章
