...
pc_base::load_sys_class('shopdataback');
class index {
	private $db;
	...
	public function show() {
		...
		if($r['status'] != 99){
			if($_username!=$r['username']){
                alert::message(-1,'無此權限');
			}
            $rs = $r;
		}else{
    ...
    public function getshop(){
        $contentpage = pc_base::load_app_class('content_tag');
        $menulist=subcat(0,0,0,1);
        $data=array();
        foreach($menulist as $r) {
            ...
            $content = $contentpage->hits_r($array);
    ...
    public function getorder(){
        ...
        foreach($menulist as $r) {
            ...
            $content = $contentpage->hits_order($array);
            array_push($data, $content);
        }
	...
	public function cookie_save(){
        ...
        alert::message(1,'讀取成功',$json_string);
	}
    ...
    public function register() {
        ...
        $link_db = pc_base::load_model(buy_model);
        $sql = array('orderid'=>$_POST['orderid'],
            'username'=>$_POST['username'],
            'shop_data'=>$_POST['shop_data'],
            'shop_fare'=>$_POST['shop_fare'],
            'shop_from'=>$_POST['shop_from'],
            'shop_total'=>$_POST['shop_total'],
            'buyer_data'=>$_POST['buyer_data'],
            'geter_data'=>$_POST['geter_data'],
            'eqno'=>$_POST['eqno'],
            'buy_prize'=>$_POST['buy_prize'],
            'buy_freight'=>$_POST['buy_freight'],
            'buy_totalnum'=>$_POST['buy_totalnum'],
            'fivenumber'=>$_POST['fivenumber'],
            'sendtime'=>$_POST['sendtime'],
            'finishmoney'=>$_POST['finishmoney']);
        $link_db->insert($sql);
        alert::message(1,'訂單已完成 請等候管理員確認');
