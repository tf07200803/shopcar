...
class content_tag {
    ...
    public function hits_r($data) {
        ...
        if($this->category[$catid]['child']) {
            $catids_str = $this->category[$catid];
            $pos = strpos($catids_str,',')+1;
            $catids_str = substr($catids_str, $pos);
            $sql .= " AND catid IN ($catids_str)";
        } else {
        ...
        $result['data'] = $this->db->listinfo($sql,'id desc',$_GET['page'],$data['limit']);
        $result['pages']=$this->db->pages;


        //$result = $this->db->select($sql, '*', $data['limit'],'id DESC','','');
        /*foreach ($ids_array as $id) {
            if($result[$id]['title']!='') {
                $array[$id] = $result[$id];
                $array[$id] = array_merge($array[$id], $hits[$id]);
    ...
    public function hits_order($data) {
        ...
        $sql='status!=99 AND status!=66 AND username=\''.$_username.'\' AND catid='.$catid;
        $this->db->table_name = $this->tablename;
