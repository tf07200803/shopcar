...
class content extends admin {
	...
	public function deleteforid(){
        ...
        $id=$_POST['shopid'];
        $link_db->delete(array('shopid'=>$id));
        alert::message(1,'種類刪除');

        //$this->position_data_db->delete(array('id'=>$id,'catid'=>$catid,'module'=>'content'));
	}

	public function deleteforeqno(){
        $link_db = pc_base::load_model(list_model);
        $id=$_POST['eqno'];
        $link_db->delete(array('eqno'=>$id));
        alert::message(1,'種類刪除');
	}
	...
	public function addlist(){
        ...
        if(isset($_POST['list'])){
            ...
            $array=json_decode(stripslashes($_POST['list']));
            $n=0;
           foreach($array as $field=>$value) {
           		//var_dump($value->shopname);
               $sql = array('content'=>$value->content,
                   'number'=>$value->number,
                   'money'=>$value->money,
                   'shopid'=>$_POST['shopid'],
                   'eqno'=>$_POST['eqno'].$n);
	...
	public function edit() {
			...
			if(isset($_POST['dosubmit']) || isset($_POST['dosubmit_continue'])) {
                ...
                $array=json_decode(stripslashes($_POST['list']));
                foreach($array as $field=>$value) {
                    $link_db->update(array('content'=>$value->content,'number'=>$value->number,'money'=>$value->money), array('eqno'=>$value->eqno));
                }
	...
	public function editlist(){
        $id = intval($_GET['id']);
        $this->link_db = pc_base::load_model('list_model');
        $infos = $this->link_db->listinfo(array('shopid'=>$id),'id ASC',1,300);
        alert::message(1,'',$infos);
	}

    public function shopdataback($shop){
	...
	public function public_preview() {
		...
		//art.dialog({lock:false,title:'管理操作',mouse:true, id:'content_m', content:'<span id=cloading ><a href=\'javascript:ajax_manage(1)\'>通过审核</a> | <a href=\'javascript:ajax_manage(2)\'>退稿</a> |　<a href=\'javascript:ajax_manage(3)\'>删除</a></span>',left:'100%',top:'100%',width:200,height:50,drag:true, fixed:true});
		...
		<div style='position: absolute; top: 20px;'><a href='javascript:ajax_manage(1)'>通过审核</a></div>
		<div style='position: absolute; top: 40px;'><a href='javascript:ajax_manage(2)'>退稿</a></div>
		<div style='position: absolute; top: 60px;'><a href='javascript:ajax_manage(3)'>删除</a></div>
