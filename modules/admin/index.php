...
class index extends admin {
	...
	public function init () {
		...
		if($type){
			$admininfo['admin_username']=$admin_username;
			$admininfo['userid']=$userid;
            $admininfo['pc_hash']=$_SESSION['pc_hash'];
			alert::message(1,'',$admininfo);
		}else{
	...
	public function login() {
		if(isset($_GET['dosubmit'])) {
			...
			if($r['password'] != $password) {
				...
				if($type){
					alert::message(-1,L('password_error',array('times'=>$times)));
				}else{
			...
			if($type){
				$admininfo['admin_username']=$username;
				$admininfo['userid']=$r['userid'];
				alert::message(1,L('login_success'),$admininfo);
			}else{
	...
	public function upload(){
        ...
        if($_POST['uploadpic']=='上传'){
			...
			if($type=='webtype'){
                if(!in_array($_FILES["uploads"]["type"],$tp)){
                    ...
                    $a=explode(".",$_FILES["uploads"]["name"]);  //截取文件名跟后缀
                ...
                alert::message(1,'圖片上傳成功',$imageSrc);
			}else if($type=='base64'){
